...
<%
    ...
    String sJWKS2 = labels.getString("jwks2.pub");
    String sSignHeader = labels.getString("sign.header");
    String sSignPayload = labels.getString("sign.payload");
%>
...
<main class="container">
    ...
    <ul>
        ...
        <li><a href="sign"    data-tooltip="Run sign servlet with demo data">sign servlet</a>       | <a href="#samplesC"  data-tooltip="Sign samples">Sign samples</a> </li>
...
/
    ...
    <details open>
        <summary role="button" class="secondary">Sample B2: Oracle APEX (any version) verify with apex_web_service.make_rest_request</summary>
        <pre>
set serveroutput on size unlimited define off
declare
  p_wallet_path     varchar2(1000);  -- := 'file:?/_wallet/';
  p_wallet_password varchar2(1000);  -- := 'changeit';
  p_url             varchar2(1000)  := 'http(s)://server:port${pageContext.request.contextPath}/verify';
  p_proxy_override  varchar2(1000);  -- := 'http://localhost:8888';
  l_result          clob;
  l_jwkSet          clob := '<%= sJWKS %>';
  l_securityEventToken clob := '<%= sDataJws %>';
  l_keyId           varchar2(200) := '<%= sKeyJws %>';
...
/
    ...
    <header>
        <h3 id="samplesC">Sample calls for sign a set</h3>
    </header>
    <!-- Secondary -->
    <details open>
        <summary role="button" class="secondary">Sample C1: Post form to sign (multipart/form-data)</summary>
        <form method="post" action="sign" enctype="multipart/form-data">
            <div class="grid">
                <label for="jwkSet">
                    jwkSet with private key
                    <textarea type="text" id="jwkSet2" name="jwkSet" placeholder="enter jwk set / public + private key list (format: json)" required><%= sJWKS2 %></textarea>
                </label>
                <label for="header">
                    header
                    <textarea type="text" id="header" name="header" placeholder="enter header (format: json)" required><%= sSignHeader %></textarea>
                </label>
                <label for="payload">
                    payload / claimset
                    <textarea type="text" id="payload" name="payload" placeholder="enter header (format: json)" required><%= sSignPayload %></textarea>
                </label>
            </div>
            <button type="submit">Submit</button>
        </form>
    ...
    <details open>
        <summary role="button" class="secondary">Sample C2: Oracle APEX (any version) sign with apex_web_service.make_rest_request</summary>
        <pre>
set serveroutput on size unlimited define off
declare
  p_wallet_path     varchar2(1000);  -- := 'file:?/_wallet/';
  p_wallet_password varchar2(1000);  -- := 'changeit';
  p_url             varchar2(1000)  := 'http(s)://server:port${pageContext.request.contextPath}/sign';
  p_proxy_override  varchar2(1000);  -- := 'http://localhost:8888';
  l_result          clob;
  l_jwkSet          clob := '<%= sJWKS2 %>';
  l_header          clob := '<%= sSignHeader %>';
  l_payload         clob := '<%= sSignPayload %>';
  l_boundary        varchar2(100) := '972D11119A46B891';
